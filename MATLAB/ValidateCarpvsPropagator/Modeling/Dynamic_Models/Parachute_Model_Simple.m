classdef Parachute_Model_Simple < Dynamic_Model
    ...
    % ========== WORKAROUND: DISTRIBUTE DRAG TO BOTH BODIES ==========
    % Calculate total system mass
    m_total = obj.m_payload + obj.m_parachute;
    
    % Distribute parachute drag force proportionally to both bodies
    % This approximates a rigid riser connection
    mass_ratio_payload = obj.m_payload / m_total;
    mass_ratio_para = obj.m_parachute / m_total;
    
    % Original approach:
    % F_p = F_g_p + F_d_p + F_r_p;
    % F_c = F_g_c + F_d_c + F_r_c;
    
    % NEW: Distribute drag to both bodies based on mass
    F_p = F_g_p + F_d_p + F_r_p + mass_ratio_payload * F_d_c;
    F_c = F_g_c + mass_ratio_para * F_d_c + F_r_c;
    % ================================================================
    
    M_p = cross(obj.payload.P_attach_B, F_r_p);
    M_c = cross(obj.parachute.P_attach_B, F_r_c);
end
