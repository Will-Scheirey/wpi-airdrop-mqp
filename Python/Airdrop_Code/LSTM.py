...
base_loc = "/Users/paigerust/wpi-airdrop-mqp/Python/Datasets"
...
def main():
    ...
    #         # ---- randomized sklearn split ----
    #         # Make sure Data/Labels are aligned and same length
    #         n = min(len(Data), len(Labels))
    #         X = Data[:n]
    #         y = Labels[:n]

    #         # If you have direction labels, align them too
    #         if has_dir:
    #             wdir = ground_truth_wdir[:n]  # or drone_dir[:n] if that's the "label" you want aligned
    #             X_train, X_test, y_train, y_test, wdir_train, wdir_test = train_test_split(
    #                 X, y, wdir,
    #                 train_size=ti,
    ...
    #         # Expand dims for LSTM: (samples, timesteps=1, features)
    #         trainData  = np.expand_dims(X_train, 1)
    #         testData   = np.expand_dims(X_test, 1)
    #         trainLabels = y_train
    #         testLabels  = y_test

    #         print("length of training: " + str(len(trainLabels)))
    #         print("length of validation: " + str(len(testLabels)))
    #         print("training) wind min: %.03f wind max: %.03f wind mean: %.02f" % (min(trainLabels), max(trainLabels), np.mean(trainLabels)))
    #         print("validation) wind min: %.03f wind max: %.03f wind mean: %.02f" % (min(testLabels), max(testLabels), np.mean(testLabels)))

    #         # Use the randomized direction split for validation metrics / saving
    #         if has_dir:
    #             validation_wind_dir = wdir_test
    #             # if you want drone_dir to correspond to the test set too:
    #             drone_dir = wdir_test
    #         # ---- end sklearn split ----

    #         model = build_LSTM(Data.shape[1])
    ...
    #         pred_cols = [f'{drone_pre}_predicted_wspd', f'{drone_pre}_est_wdir', 'ground_wspd', 'ground_wdir']
    ...
    #                 'ground_wspd': actual_wspd,
    #                 'ground_wdir': validation_wind_dir
    #             })
    #         except:
    #             pred_df = pd.DataFrame({
    #                 f'{drone_pre}_predicted_wspd': drone_wspd_pred,
    #                 f'{drone_pre}_est_wdir': drone_dir,
    #                 'ground_wspd': list(itertools.chain.from_iterable(actual_wspd)),
    #                 'ground_wdir': list(itertools.chain.from_iterable(validation_wind_dir))
    #             })
    ...
    # title = "LSTM MATLAB Test 1"
    # labels = ["Payload Wspd", "True Wspd"]
    # y_lims = [0, 11]
...
# Creates windows based on input data to train LSTM
def make_windows(X, y, lookback=LOOKBACK, stride=STRIDE):
    X_w, y_w = [], []
    T = min(len(X), len(y))
    X = X[:T]
    y = y[:T]
...
def train_val_split(X, y, fraction=TRAIN_VAL_SPLIT):
    T = min(len(X), len(y))
    X = X[:T]
    y = y[:T]
